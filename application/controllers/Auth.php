...
class Auth extends CI_Controller
{
	...
	public function create_user()
	{
		...
		if ($this->form_validation->run() === TRUE)
		{
			$email = $_SESSION["emailMaker"];
			//$identity = ($identity_column === 'email') ? $email : $this->input->post('identity');
			$password = $this->input->post('password');

			$additional_data = [
				'first_name' => $_SESSION["adresseMaker"],
				'last_name' => $_SESSION["telMaker1"],
				'company' => $_SESSION["telMaker2"],
				'phone' => $_SESSION["siteMaker"],
			];
		}
		if ($this->form_validation->run() === TRUE && $this->ion_auth->register($email, $password,$email,  $additional_data))
		{
			...
			/*$this->load->model('insertData');
			$this->insertData->updateProd($codeBarProd);*/
			$this->session->set_flashdata('message', $this->ion_auth->messages());
            $this->ion_auth->login($email, $password);
			$this->session->unset_userdata(
				$_SESSION["adresseMaker"],
				$_SESSION["telMaker1"],
				$_SESSION["telMaker2"],
				$_SESSION["siteMaker"]
			);
